...
def main(argv=None):
    ...
    if os.path.isfile(cachename) and os.path.getmtime(cachename) > os.path.getmtime(arg.input):
        ...
        data = sensors(arg.input, sensor.dateTimeTo5Minute() if arg.by5min else sensor.dateTimeToMinute(
        ), lambda n: 'Extérieur' if n == 'Extérieur' else 'Intérieur')
    ...
    for n, fd in sorted(data.toMultiFigures().items()):
        ...
        for sn, d in sorted(fd.items()):
            output = os.path.join(targetfolder, sn.translate(
                str.maketrans('/:\\', '---')) + ".html")
